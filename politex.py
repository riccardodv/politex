...
for i in tqdm(range(max_iterations)):
    ...
    for t in range(tau):
        ...
        if done:
            ...
            episodes_lengths.append(
                episode_length
            )  # TODO this is basically the total_rewards-1 since reward is +1 for each time step, do we need it?
    ...
    Q_hat = i * Q_hat / (i + 1) + Q / (
        i + 1
    )  # TODO check this is the right fomula for cumulative average
    ...
    if (i + 1) % print_every == 0:
        ...
        print(
            f"Iteration {i + 1}/{max_iterations} | Last total reward: {total_reward} | "
